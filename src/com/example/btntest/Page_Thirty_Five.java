...
public class Page_Thirty_Five extends Activity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		thirty_fiveNext = (ImageButton)findViewById(R.id.thirty_fiveNext);
		...
		thirty_fiveNext.setOnClickListener(new OnClickListener() {
	...
	/*
            ...
            public void onReceive(Context arg0, Intent arg1) {
                switch (getResultCode())
                {
                    case Activity.RESULT_OK:
                        Toast.makeText(getApplicationContext(), "문자가 전송되었어요.", 
        ...
        */
        ...
        if(!phoneNumber1.equals("연락처"))
        	...
        	Toast.makeText(getApplicationContext(), thirty_five_personName1.getText().toString() + "에게 문자가 전송되었어요.", Toast.LENGTH_SHORT).show();
  
		if(!phoneNumber2.equals("연락처"))
    		sms.sendTextMessage(phoneNumber2, null, message, null, null);
			Toast.makeText(getApplicationContext(), thirty_five_personName2.getText().toString() + "에게 문자가 전송되었어요.", Toast.LENGTH_SHORT).show();

		if(!phoneNumber3.equals("연락처"))
			sms.sendTextMessage(phoneNumber3, null, message, null, null);
			Toast.makeText(getApplicationContext(), thirty_five_personName3.getText().toString() + "에게 문자가 전송되었어요.", Toast.LENGTH_SHORT).show();
	}
	
	private void DialogSimple(){
		
		 AlertDialog.Builder alt_bld = new AlertDialog.Builder(this);
		    alt_bld.setMessage("1명도 충분해!").setCancelable(false).setPositiveButton("OK", new DialogInterface.OnClickListener() {
